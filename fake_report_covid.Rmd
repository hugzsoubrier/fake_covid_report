...
output: 
  html_document:
    toc: true
---
# Do the set - up of the document data libraries 
...
```{r data loading & cleaning, echo = F, eval = T, message = F}
#read data
...
clean_data <- clean_data %>% 
  mutate( hospitalized = recode(hospitalized, "yes" = "hospitalised", "no" = "not hospitalised"))
...
clean_data_sf <- left_join(clean_data, vector, by = c("case_zip" = "ZipCode"))


clean_data_sf %>% group_by(weeks = floor_date(pos_sampledt_FALSE, "week", 
                               week_start = getOption("lubridate.week.start",1)), 
  ...
  mutate(AR = round(digits = 2, n/Population * 10000))
