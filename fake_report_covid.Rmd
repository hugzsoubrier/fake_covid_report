...
This document is a fake epidemiological report using data provided by the [Applied Epi](https://appliedepi.org) group using a fake COVID linelist ([data available on Github](https://github.com/appliedepi/epiRhandbook_eng)). 

I have used these data to create various descriptive analysis. Most of the work here is inspired from the great [Epidemiologist R handbook](https://www.epirhandbook.com/en/), but also from other resources that I have used to learn R. I use pipes and all of the tidyverse has I have learned in: [R for Data Science](https://r4ds.had.co.nz/index.html) and other resources available on [Rstudio website](https://www.rstudio.com/resources/books/). 

For data visualisation I have decided to focuss on interactive graphs using the ``highcharter`` package a wrapper for the [highchart library](https://jkunst.com/highcharter/index.html). I have been working with highcharter for some time now but I used it as an opportunity to learn more, and apply it to a real life scenarios. I do really enjoy the adds-on of interactivity in data analysis.
...
```{r hosp rates per age_group table}
...
Hopsitalisations rates remained similar between **Male**  ( `r fmt_ci_df(filter(hosp_rate_sex, Sex == "male"), 4, 5, 6 )` ) and **Female** ( `r fmt_ci_df(filter(hosp_rate_sex, Sex == "male"), 4, 5, 6 )` ). 
...
clean_data %>% 
  ...
  filter(case_zip %in% pop_data$ZipCode) %>% 
  
  drop_na(pos_sampledt_FALSE) %>%
  
  mutate(month = format(pos_sampledt_FALSE, "%Y-%m")) %>% 
