...
## Test symptoms visualisation 

```{r dependency wheel symptoms}

#create dummy data from subset of big data
dummy_data <- data %>% select(contains("sym"), 
                -contains("dt_"), 
                -sym_resolved) %>%
  
  #recode all var from Yes to 1
  mutate(across(.cols = everything(), ~ifelse(.x == "Yes", 1, 0)))
...
for (var1 in c(names(dummy_data))) {
  ...
  for(var2 in c(names(dummy_data))) { 
    ...
    #if this row contains inverted values from a previous row then skip itteration 
    if(new_row$from %in% table$to & new_row$to %in% table$from) {
      
      next
      
    } else{
      #bind new_row to the table
...
#create a function that computes the N of cases with one symptoms vs another in dummy_data
...
#vectorize the function to use it in mutate
weight_var1_var2_V <- Vectorize(weight_var1_var2)


#apply our weight function to all combination of the table 
table <- table %>% 
  mutate(weight = weight_var1_var2_V(from, to))

#plot
highchart() %>% 
  hc_add_series(data = table, type = "dependencywheel") %>% 
  
  hc_title(text = paste0("Cases by symptoms"), 
           align = "left", 
           style = list(useHTML =T, color = "#34495E", fontSize= 13)) %>%
